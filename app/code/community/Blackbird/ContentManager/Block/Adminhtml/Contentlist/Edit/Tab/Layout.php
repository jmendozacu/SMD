...
class Blackbird_ContentManager_Block_Adminhtml_Contentlist_Edit_Tab_Layout extends Mage_Adminhtml_Block_Widget_Form
{
    protected function _prepareForm()
    {
        ...
        $fieldset = $form->addFieldset('content_layout', array('legend'=>Mage::helper('contentmanager')->__('Step 1 - Select your layout')));            
        ...
        //new renderer
       $form->getElement('layout')->setRenderer($this->getLayout()->createBlock('contentmanager/adminhtml_widget_form_renderer_fieldset_layout_list'));
        
        $fieldset2 = $form->addFieldset('contentlist_layout_items', array('legend'=>Mage::helper('contentmanager')->__('Step 2 - Drag and drop items in your layout')));
        $fieldset2->addField('layout_items', 'text' , array(
            'label'     => Mage::helper('contentmanager')->__('Layout items'),
            'name'      => 'layout_items',
        ));
        //new renderer
        $form->getElement('layout_items')->setRenderer($this->getLayout()->createBlock('contentmanager/adminhtml_widget_form_renderer_fieldset_layout_itemslist'));
        ...
        if ( Mage::getSingleton('adminhtml/session')->getContentListData() )
        {
            ...
            if (!Mage::registry('contentlist_data')->getRootTemplate()) {
                Mage::registry('contentlist_data')->setRootTemplate(Mage::getSingleton('page/source_layout')->getDefaultValue());
            }
            $form->setValues(Mage::registry('contentlist_data')->getData());
        }
