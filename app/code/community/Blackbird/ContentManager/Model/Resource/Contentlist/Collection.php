...
class Blackbird_ContentManager_Model_Resource_Contentlist_Collection extends Mage_Core_Model_Mysql4_Collection_Abstract
{
    /**
    ...
    {
        parent::_construct();
        $this->_init('contentmanager/contentlist');
    ...
    protected function _afterLoad()
    {
        if (isset($this->_previewFlag) && $this->_previewFlag) {
            $items = $this->getColumnValues('cl_id');
            $connection = $this->getConnection();
            if (count($items)) {
                $select = $connection->select()
                        ->from(array('cps'=>$this->getTable('contentmanager/contentlist_store')))
                        ->where('cps.cl_id IN (?)', $items);

                if ($result = $connection->fetchPairs($select)) {
                    foreach ($this as $item) {
                        if (!isset($result[$item->getData('cl_id')])) {
                            continue;
                        }
                        if ($result[$item->getData('cl_id')] == 0) {
                            $stores = Mage::app()->getStores(false, true);
                            $storeId = current($stores)->getId();
                            $storeCode = key($stores);
                        } else {
                            $storeId = $result[$item->getData('cl_id')];
    ...
    protected function _renderFiltersBefore()
    {
        if ($this->getFilter('store')) {
            $this->getSelect()->join(
                array('store_table' => $this->getTable('contentmanager/contentlist_store')),
                'main_table.cl_id = store_table.cl_id',
                array()
            )->group('main_table.cl_id');
    ...
    protected function _initSelect()
    {
        ...
        if(Mage::app()->getStore()->getId() == 0) //is in admin
        {
            ...
            {
                if(($adminStoreFilter == $store->getId() || !$adminStoreFilter))
                {
        ...
        if($allowedStoreIds)
        {
            ...
            $joinCondition  = array(
                'store.cl_id = main_table.cl_id'
            );
            $this->getSelect()
                ->joinLeft(
                    array('store' => $this->getTable('contentmanager/contentlist_store')),
