...
class AW_Autorelated_Model_Blocks_Rule extends Mage_CatalogRule_Model_Rule
{
    ...
    protected function prepareRules($rules = null, $aggregator = null, $condition = null)
    {
        ...
        $aggregators = array('any', 'all');
        $sql = null;
        foreach ($rules as $rule) {
            if (!is_array($rule) && !in_array($rule, $aggregators)) {
                ...
                $suffix = ' AND ';
                if (in_array($rule, $aggregators)) {
                    $this->_aggregator = $rule;
                    continue;
                }
                if ($this->_aggregator == 'any') {
                    $suffix = ' OR ';
                }
                ...
                if (isset($sql)) {
                    $sql.= $suffix . $sign . '(' . $this->prepareRules($rule, $this->_aggregator, $condition) . ')';
                } else {
                    $sql = '(' . $this->prepareRules($rule, $this->_aggregator, $condition) . ')';
                }
            }
        }
        return $sql;
