    ...
    public function handleBlockOutput($observer)
    {
        ...
        if (Mage::getStoreConfig(
            ...
            if ($onepageBillingClass == get_class($block)
                ...
                $flag = true;
                $isAmastyOSCEnabled = false;
                if (Mage::helper('ambase')->isModuleActive('Amasty_Scheckout')) {
                    $isAmastyOSCEnabled = Mage::getStoreConfig('amscheckout/general/enabled');
                }
                if (false === strpos($html, 'amcustomerattr') && !$isAmastyOSCEnabled) {
