...
class Webtise_Gallery_Model_Resource_Gallery_Attribute_Backend_Media extends Mage_Core_Model_Resource_Db_Abstract
{
    const GALLERY_TABLE       = 'gallery/gallery_attribute_media_gallery';
    const GALLERY_VALUE_TABLE = 'gallery/gallery_attribute_media_gallery_value';
    const GALLERY_IMAGE_TABLE = 'gallery/gallery_attribute_media_gallery_image';

    protected $_eventPrefix = 'gallery_gallery_attribute_backend_media';
    ...
    /**
     * Load gallery images for gallery using reusable select method
     *
     * @param Webtise_Gallery_Model_Gallery $gallery
     * @param Webtise_Gallery_Model_Gallery_Attribute_Backend_Media $object
     * @return array
     */
    public function loadGallery($gallery, $object)
    {
        ...
        if ($eventObjectWrapper->hasGalleryIdsOverride()) {
            $galleryIds = $eventObjectWrapper->getGalleryIdsOverride();
        } else {
            $galleryIds = array($gallery->getId());
        }

        $select = $this->_getLoadGallerySelect($galleryIds, $gallery->getStoreId(), $object->getAttribute()->getId());
     ...
     *
     * @param array $result
     * @return Webtise_Gallery_Model_Resource_Gallery_Attribute_Backend_Media
     */
    ...
    protected function _getLoadGallerySelect(array $productIds, $storeId, $attributeId) {
        ...
        $select = $adapter->select()
            ->from(
                ...
                array('value_id', 'value AS file', 'gallery_id' => 'entity_id')
            )
            ->joinLeft(
                ...
                array('label', 'related_product_ids', 'image_specific_url', 'tag_ids', 'position','disabled')
            )
    ...
    protected function _getAttributeId() {
        if(is_null($this->_attributeId)) {
            $attribute = Mage::getModel('eav/entity_attribute')
                ->loadByCode(Webtise_Gallery_Model_Gallery::ENTITY, 'media_gallery');
